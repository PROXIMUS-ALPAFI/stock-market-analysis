...
def log_action(symbol, action):
    conn = sqlite3.connect("data/user_history.db")
    cursor = conn.cursor()
    cursor.execute("INSERT INTO history (symbol, action) VALUES (?, ?)", (symbol, action))
    conn.commit()
    conn.close()
...
def fetch_history():
    conn = sqlite3.connect("data/user_history.db")
    cursor = conn.cursor()
    cursor.execute("SELECT id, symbol, action, timestamp FROM history ORDER BY timestamp DESC")
...
def get_stock_data():
    ...
    # Log action
    log_action(symbol, "Fetched stock data")

    # Convert the first few rows to a dictionary and return it as part of the response
    df_head = df[['Date', 'Close', 'SMA_50', 'SMA_200', 'DMA_50_200', 'RSI', 'MACD', 'Signal_Line', 'Upper_Band', 'Lower_Band', 'Volume_Change', 'Sentiment', 'Future Close']].tail().to_dict(orient='records')
...
def train_model():
    ...
    features = ['SMA_50', 'SMA_200', 'DMA_50_200', 'RSI', 'MACD', 'Signal_Line', 'Upper_Band', 'Lower_Band', 'Volume_Change', 'Sentiment']
    X = df[features]
    y = df['Future Close']
    ...
    log_action(symbol, "Trained model")
